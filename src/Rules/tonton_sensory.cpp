...
namespace TonTon
{
static length_m PredictedEyeDiameter(Analysis_Physical const& physical);
...
static Analysis_Vision ComputeVision(Input const& in, Scratch const& s) {
	...
	// no verticies associated with eye.
	if(max_eye_diameter == 0.0)
	{
		max_eye_diameter = PredictedEyeDiameter(s.physical);
	}
...
static length_m PredictedEyeDiameter(Analysis_Physical const& physical)
{
	constexpr density_kg_m3 EYE_DENSITY = 1050.0f; // kg/m³
	
	length_m accumulator = 0;
	
	uint32_t total_contributions =
	ForTopLevelClade(physical.clade,
	// true if we handle the case false if not.
	// if we don't handle it it will be passed to the parent clade
	// we could have cephalapodia and reptilia and aves if we're a mythical creature.
	   [&](CladeFlags clade) -> bool
	{		switch(clade)
		{
		case CladeFlags::AVES:
		{
			// Eye mass ∝ (body_mass)^0.68
			// Assuming eye density ~1000 kg/m³ (similar to tissue)
			// Eye mass (kg) = 0.68 * body_mass^0.68 (empirical coefficient needs adjustment)
			constexpr float BASE_COEFF = 0.000013f; // Empirically derived constant
			mass_kg eye_mass_kg = BASE_COEFF * std::pow(float(physical.body_mass_kg), 0.68f);
			
			// Convert mass to volume (assuming spherical eye)
			volume_m3 eye_volume_m3 = eye_mass_kg / EYE_DENSITY;
			
			// Diameter from sphere volume: V = (4/3)πr³
			length_m eye_radius = cbrt((3.0f * eye_volume_m3) / (4.0f * M_PI));
			length_m eye_diameter = eye_radius * 2.0f;
			
			// Special cases: raptors and owls
			if (HasFlag(physical.niche, (NicheFlags::PREDATOR | NicheFlags::AERIAL))) {
				eye_diameter *= 1.4f; // Raptors
			}
			if (HasFlag(physical.niche, NicheFlags::NOCTURNAL)) {
				eye_diameter *= 2.2f; // Owls
			}
		...
		case CladeFlags::MAMMALIA:
		case CladeFlags::UNGULATA:
		case CladeFlags::EQUIDAE:
		case CladeFlags::CETACEA:
		{
			// For mammals, eye size is more constrained by brain/head size
			// Rough approximation: brain mass ≈ 0.01 * body_mass^0.75 (Kleiber's law variant)
			float brain_mass_kg = 0.01f * std::pow(float(physical.body_mass_kg), 0.75f);
			
			// Eye mass scales roughly isometrically with brain mass for mid-sized mammals
			// Eye mass ≈ 0.003 * brain_mass (roughly 0.3% of brain mass)
			mass_kg eye_mass_kg = 0.003f * brain_mass_kg;
			
			// Cap for very large brains (>220g) - diminishing returns
			if (brain_mass_kg > 0.22f) {
				eye_mass_kg = 0.003f * 0.22f; // Eye mass plateaus
			}
			...
			// Nocturnal mammals have larger eyes
			if (HasFlag(physical.niche, NicheFlags::NOCTURNAL)) {
				eye_diameter *= 1.3f;
			}
		...
		case CladeFlags::INSECTA:
		{
			// Compound eyes show negative allometry
			// Strong scaling with head/body size ratio
			// Rough approximation for compound eye diameter
			length_m head_diameter = cbrt(physical.body_volume_m3) * 0.3f; // Head ~3-10% of body
			length_m eye_diameter = head_diameter * 0.4f; // Eyes are significant portion of head
			
			// Flying insects (especially predators) have relatively larger eyes
			if (HasFlag(physical.niche, NicheFlags::AERIAL)) {
				eye_diameter *= 1.3f;
			}
		...
		case CladeFlags::CEPHALOPODA:
		{
			// Cephalopods have the largest eyes relative to body size among invertebrates
			// Giant squid have eyes ~27cm diameter
			// Rough scaling: eye diameter ∝ body_length^0.4
			length_m eye_diameter = 0.15f * std::pow(float(physical.body_length_m), 0.4f);
			
			// Deep-sea species have larger eyes
			if (HasFlag(physical.niche, NicheFlags::DEEP_SEA)) {
				eye_diameter *= 1.4f;
			}
		...
		case CladeFlags::PISCES:
		{
			// Fish eyes scale with negative allometry
			// Axial length follows logarithmic relationship with body weight
			// Rough approximation: eye_diameter ∝ body_mass^0.3
			length_m eye_diameter = 0.025f * std::pow(float(physical.body_mass_kg), 0.3f);
			
			// Deep sea fish have larger eyes
			if (HasFlag(physical.niche, NicheFlags::DEEP_SEA)) {
				eye_diameter *= 1.8f;
			}
		...
		case CladeFlags::REPTILIA:
		case CladeFlags::CHELONIA:
		{
			// Reptiles have smaller eyes than birds of similar size
			// Similar scaling to general vertebrates
			length_m eye_diameter = 0.018f * std::pow(float(physical.body_mass_kg), 0.35f);
			
			accumulator += eye_diameter;
			return true;
		}
		
		case CladeFlags::AMPHIBIA:
		{
			// Amphibians have relatively large eyes for their size
			length_m eye_diameter = 0.022f * std::pow(float(physical.body_mass_kg), 0.33f);
			
			accumulator += eye_diameter;
			return true;
		}
		
		case CladeFlags::ARACHNIDA:
		{
			// Spiders have multiple small eyes
			// Return diameter of largest principal eyes
			length_m head_size = cbrt(physical.body_volume_m3) * 0.25f;
			length_m eye_diameter = head_size * 0.15f; // Principal eyes
			
			// Jumping spiders have proportionally larger eyes
			if (HasFlag(physical.niche, NicheFlags::PREDATOR)) {
				eye_diameter *= 1.5f;
			}
		...
		case CladeFlags::CRUSTACEA:
		{
			// Crustaceans have compound or stalked eyes
			length_m body_diameter = cbrt(physical.body_volume_m3);
			length_m eye_diameter = body_diameter * 0.08f;
			
			accumulator += eye_diameter;
			return true;
		}
		
		case CladeFlags::CHORDATA:
		{
	...
	return accumulator / total_contributions;
}
